...
export default function AdminWeeklyTrackingPage() {
  ...
  const loadData = async (semaine, dept) => {
    if (!dept) return;
    setLoading(true);
    try {
      const res = await dashboardService.getWeeklyTracking(semaine, dept);
      setData(res.data);
    } catch {
  ...
  useEffect(() => {
    if (selectedDept) {
      loadData(currentMonday, selectedDept);
    } else {
      setData(null);
    }
  }, [currentMonday, selectedDept]);
  ...
  return (
    <Box className="fade-in">
      <PageHeader title="Suivi hebdomadaire" description="Suivez les fiches de suivi par departement, filiere et niveau" />
        ...
        <Card>
          <CardContent sx={{ textAlign: 'center', py: 4 }}>
            <Typography color="text.secondary">
              Selectionnez un departement pour voir le suivi hebdomadaire.
            </Typography>
          </CardContent>
        </Card>
      ) : (
