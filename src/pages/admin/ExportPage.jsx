...
import { dashboardService, unitesEnseignementService, usersService, departementsService } from '../../api/services';
...
export default function AdminExportPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [ueRes, usrRes, deptRes] = await Promise.all([
  ...
  useEffect(() => {
    loadRecap();
  }, [dateDebut, dateFin, selectedDepartement, selectedFiliere, selectedNiveau, selectedSemestre, refreshKey]);
  ...
  const handleDepartementChange = (val) => {
  ...
  return (
    <Box className="fade-in">
      <PageHeader title="Export et recapitulatif" description="Telechargez les bilans de cours en format Excel — Vue globale" />
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          {/* Departement / Filiere / Niveau / Semestre */}
